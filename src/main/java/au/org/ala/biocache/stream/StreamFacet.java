...
public class StreamFacet implements ProcessInterface {
    ...
    public StreamFacet(SearchDAOImpl searchDAO, DownloadDetailsDTO downloadDetails, SpatialSearchRequestParams request,
        ...
        String name = downloadDetails.getRequestParams().getFacets()[0];

        String[] header = new String[]{name};

        String facetName = downloadDetails.getRequestParams().getFacets()[0];
        boolean shouldLookupTaxon = lookupName && (facetName.contains("_guid") || facetName.contains("_lsid"));
        boolean shouldLookupAttribution = lookupName && facetName.contains("_uid");

        if (shouldLookupTaxon) {
            header = searchDAO.speciesLookupService.getHeaderDetails(name, includeCount, includeSynonyms);
